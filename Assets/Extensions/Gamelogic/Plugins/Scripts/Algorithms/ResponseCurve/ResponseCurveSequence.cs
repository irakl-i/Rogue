...
namespace Gamelogic.Extensions.Algorithms
{
	...
	public class ResponseCurveFloatSequence : ResponseCurveBase<IList<float>>
	{
		...
		/// <param name="outputSamples">
		///     Each item in the output samples should have the same number
		///     of elements.
		/// </param>
		...
		/// </summary>
		/// <example>
		///     If the min sequence is (0, 1, 2) and the max sequence is (9, 7, 5), the interpolation
		...
		{
			if (outputSampleMin.Count != outputSampleMax.Count)
				throw new ArgumentException("The numbers of elements in outputSampleMin and outputSampleMax must match");

			var output = new List<float>();

			for (var i = 0; i < outputSampleMin.Count; i++)
