...
namespace Gamelogic.Extensions.Editor.Internal
{
	...
	public static class EditorUtils
	{
		...
		{
			var colorCount = colorsProp.arraySize;

			EditorGUILayout.BeginVertical();
			EditorGUILayout.BeginHorizontal();

			var columns = (int) (position.width / 16);
			...
			if (columns > 0)
			{
				...
				for (var i = 0; i < Mathf.Min(100, colorCount); i++)
				{
					if (i != 0 && i % columns == 0)
					{
						x = position.x;
			...
			float heightOffset = -2.0f)
		{
			...
			if (columns > 0)
			{
				...
				for (var i = 0; i < Mathf.Min(maxColors, colorCount); i++)
				{
					//Makes a new row
					if (i != 0 && i % columns == 0)
					{
