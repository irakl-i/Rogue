...
{
	/// <summary>
	///     A NearestNeighbour iterator for the KD-tree which intelligently iterates and captures relevant data in the search
	///     space.
	/// </summary>
	...
	public class NearestNeighbour<T> : IEnumerator
	{
		...
		///     Threshold to apply to the search space.  Negative numbers indicate that no threshold is
		///     applied.
		/// </param>
		public NearestNeighbour(KDNode<T> pRoot, Vector2 tSearchPoint, IDistanceFunction kDistance, int iMaxPoints,
			float fThreshold)
		{
		...
		public bool MoveNext()
		{
			...
			while (pPending.Size > 0 && (pEvaluated.Size == 0 || pPending.MinKey < pEvaluated.MinKey))
			{
				...
				while (!pCursor.IsLeaf)
				{
					...
					var fDistance = kDistanceFunction.DistanceToRectangle(tSearchPoint, pNotTaken.tMinBound, pNotTaken.tMaxBound);
				...
				if (pCursor.bSinglePoint)
				{
					...
					var fDistance = kDistanceFunction.Distance(pCursor.tPoints[0], tSearchPoint);
				...
				else
				{
					...
					for (var i = 0; i < pCursor.Size; ++i)
					{
						...
						var fDistance = kDistanceFunction.Distance(pCursor.tPoints[i], tSearchPoint);
